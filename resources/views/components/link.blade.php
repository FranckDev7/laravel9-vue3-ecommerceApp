@props(['href', 'color' => 'primary', 'size' => 'md', 'fullWidth' => false])
...
@php
    ...
    $fullWidthClass = $fullWidth ? 'w-full justify-center inline-flex' : 'inline-flex';
@endphp

<a href="{{ $href }}"
    {{ $attributes->merge([
        'class' => "$fullWidthClass items-center $sizeClasses $colorClasses border border-transparent rounded-md font-semibold uppercase tracking-widest focus:outline-none focus:ring-2 focus:ring-offset-2 dark:focus:ring-offset-gray-800 transition ease-in-out duration-150"
    ]) }}>
    {{ $slot }}
</a>
